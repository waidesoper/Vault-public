...
public class VaultBarOverlay {
    ...
    onPostRender(RenderGameOverlayEvent.Post event) {
        ...
        int yOffset = player != null && player.getActiveEffects().stream().anyMatch(effectInstance ->
            effectInstance.showIcon() && effectInstance.getEffect().getCategory() != EffectType.HARMFUL) ? 28 : 18;

        if (player != null && player.getActiveEffects().stream().anyMatch(effectInstance ->
            effectInstance.showIcon() && effectInstance.getEffect().getCategory() == EffectType.HARMFUL)) yOffset += 25;
        ...
        if (VaultBarOverlay.unspentSkillPoints > 0) {
            ...
            minecraft.font.drawShadow(matrixStack, unspentPointsText, right - unspentWidth - unspentPointsWidth, yOffset, 0xFF_ffd800);
            minecraft.font.drawShadow(matrixStack, unspentText, right - unspentWidth, yOffset, 0xFF_ffffff);
        ...
        if (VaultBarOverlay.unspentKnowledgePoints > 0) {
            ...
            int unspentPointsWidth = minecraft.font.width(unspentPointsText);
            int unspentWidth = minecraft.font.width(unspentText);
            minecraft.font.drawShadow(matrixStack, unspentPointsText, right - unspentWidth - unspentPointsWidth, yOffset, 0xFF_40d7b1);
            minecraft.font.drawShadow(matrixStack, unspentText, right - unspentWidth, yOffset, 0xFF_ffffff);
//            matrixStack.popPose();
        }
        ...
        minecraft.gui.blit(matrixStack, midX + 10, bottom - 48, 1, 1, barWidth, 5);

        if (expGainedAnimation.getValue() != 0) {
            GlStateManager._color4f(1, 1, 1, expGainedAnimation.getValue());
            minecraft.gui.blit(matrixStack, midX + 9, bottom - 49, 62, 41, 84, 7);
            GlStateManager._color4f(1, 1, 1, 1);
        }

        minecraft.gui.blit(matrixStack, midX + 10, bottom - 48, 1, 7, (int) (barWidth * expPercentage), 5);

        if (expGainedAnimation.getValue() != 0) {
            GlStateManager._color4f(1, 1, 1, expGainedAnimation.getValue());
            minecraft.gui.blit(matrixStack, midX + 9, bottom - 49, 62, 49, (int) (barWidth * expPercentage), 7);
            GlStateManager._color4f(1, 1, 1, 1);
        }

        FontHelper.drawStringWithBorder(matrixStack, text, textX, bottom - 45 - minecraft.font.lineHeight, 0xFF_ffe637, 0x3c3400);
